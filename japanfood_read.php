...
<?php
	...
	$num = $_GET['num'];
	$sql = 'SELECT * FROM japan_food WHERE num='.$num;
		...
		<table width=100%>
			<tr>
				<td width=200 align=left height=20>
					<a href="japanfood_list.php?num=<?php echo $num?>"><font color=white>
					[목록보기]</font></a>
					<a href="japanfood_write.php"><font color=white>
					[글 쓰기]</font></a>
				...
				<td align=right>
				<?php
					$sql = "SELECT num FROM japan_food WHERE num > $num LIMIT 1";
					$prev_num = mysqli_fetch_assoc($connect, $sql);
					
					if($prev_num['num'])
					{
						echo "<a href=\"japanfood_read.php?num=".$prev_num['num']."\"><font color='white'>[이전]</font></a>";
					}
					...
					$sql1 = "SELECT num FROM japan_food WHERE num < $num ORDER BY num DESC LIMIT 1";
					$next_num = mysqli_fetch_assoc($connect, $sql1);

					
					if($next_num['num'])
					{
						echo "<a href=\"japanfood_read.php?num=".$next_num['num']."\"><font color='white'>[다음]</font></a>";;
					}
					else
					{
						echo "[다음]";
...
<?php
	$sql = 'UPDATE japan_food SET hit=hit+1 WHERE num='.$num;
	$result = mysqli_query($connect, $sql);
