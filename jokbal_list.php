<?php
	...
	$sql = 'select count(*) as cnt from jokbal order by num desc';
...
?>
	...
	<?php
		...
		if($page != 1) { 
			$paging .= '<li class="page page_start"><a href="./jokbal_list.php?page=1">처음</a></li>';
		}
		//첫 섹션이 아니라면 이전 버튼을 생성
		if($currentSection != 1) { 
			$paging .= '<li class="page page_prev"><a href="./jokbal_list.php?page=' . $prevPage . '">이전</a></li>';
		}
		...
		for($i = $firstPage; $i <= $lastPage; $i++) {
			if($i == $page) {
				...
				$paging .= '<li class="page"><a href="./jokbal_list.php?page='.$i.'">'.$i.'</a></li>';
		}
	}
	
	//마지막 섹션이 아니라면 다음 버튼을 생성
	if($currentSection != $allSection) { 
		$paging .= '<li class="page page_next"><a href="./jokbal_list.php?page='.$nextPage.'">다음</a></li>';
	}
	
	//마지막 페이지가 아니라면 끝 버튼을 생성
	if($page != $allPage) { 
		$paging .= '<li class="page page_end"><a href="./jokbal_list.php?page='.$allPage.'">끝</a></li>';
	}
	$paging .= '</ul>';
	...
	$sql = 'select * from jokbal order by num desc'.$sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
	$result = mysqli_query($connect, $sql);
?>
<!DOCTYPE html>
<html>
...
<table width=580 border=0 cellpadding=2 cellspacing=1 bgcolor=#FFE5C2>
	<tr height=20 bgcolor=#FFE5C2>
		...
		<td width=90 align=center>
			<a href="./jokbal_write.php" class="btnWrite btn">[글쓰기]</a>
		</td>
	</tr>		
	<?php
		if(isset($emptyData)) {
	...
	?>
		<tr>
			...
			<a href="./jokbal_read.php?num=<?php echo $row['num']?>"> </a>
			</td>
		
			<td height=20 bgcolor=white> &nbsp;
			<a href="./jokbal_read.php?num=<?php echo $row['num']?>"><?php echo $row['subject']?></a>
			</td>
		...
		<div id="searchBox">
			<form action="./jokbal_list.php" method="get">
