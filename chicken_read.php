...
<?php
	...
	$num = $_GET['num'];
	$sql = 'SELECT * FROM chicken WHERE num='.$num;
		...
		<table width=100%>
			<tr>
				<td width=200 align=left height=20>
					<a href="chicken_list.php?num=<?php echo $num?>"><font color=black>
					[목록보기]</font></a>
					<a href="chicken_write.php"><font color=black>
					[글 쓰기]</font></a>
					<a href="chicken_erase_form.php?num=<?php echo $num?>"><font color="black">
					[삭제]</font></a>
				</td>
				<td align=right>
				<?php
					$sql = "SELECT num FROM chicken WHERE num > $num LIMIT 1";
					$prev_num = mysqli_fetch_assoc($connect, $sql);
					
					if($prev_num['num'])
					{
						echo "<a href=\"chicken_read.php?num=".$prev_num['num']."\"><font color='white'>[이전]</font></a>";
					}
					...
					$sql1 = "SELECT num FROM chicken WHERE num < $num ORDER BY num DESC LIMIT 1";
					$next_num = mysqli_fetch_assoc($connect, $sql1);

					
					if($next_num['num'])
					{
						echo "<a href=\"chicken_read.php?num=".$next_num['num']."\"><font color='white'>[다음]</font></a>";;
					}
					else
					{
						echo "[다음]";
...
<?php
	$sql = 'UPDATE chicken SET hit=hit+1 WHERE num='.$num;
	$result = mysqli_query($connect, $sql);
