<?php
	...
	$sql = 'select count(*) as cnt from base_food order by num desc';
...
?>
	...
	<?php
		...
		if($page != 1) { 
			$paging .= '<li class="page page_start"><a href="./basefood_list.php?page=1">처음</a></li>';
		}
		//첫 섹션이 아니라면 이전 버튼을 생성
		if($currentSection != 1) { 
			$paging .= '<li class="page page_prev"><a href="./basefood_list.php?page=' . $prevPage . '">이전</a></li>';
		}
		...
		for($i = $firstPage; $i <= $lastPage; $i++) {
			if($i == $page) {
				...
				$paging .= '<li class="page"><a href="./basefood_list.php?page='.$i.'">'.$i.'</a></li>';
		}
	}
	
	//마지막 섹션이 아니라면 다음 버튼을 생성
	if($currentSection != $allSection) { 
		$paging .= '<li class="page page_next"><a href="./basefood_list.php?page='.$nextPage.'">다음</a></li>';
	}
	
	//마지막 페이지가 아니라면 끝 버튼을 생성
	if($page != $allPage) { 
		$paging .= '<li class="page page_end"><a href="./basefood_list.php?page='.$allPage.'">끝</a></li>';
	}
	$paging .= '</ul>';
	...
	$sql = 'select * from base_food order by num desc'.$sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
	$result = mysqli_query($connect, $sql);
?>
<!DOCTYPE html>
<html>
...
<table width=580 border=0 cellpadding=2 cellspacing=1 bgcolor=#777777>
	...
	?>
		...
		<div id="btnSet">
			<a href="./basefood_write.php" class="btnWrite btn">글쓰기</a>
		</div>
		<div id="paging">
			<?php echo $paging ?>
		</div>
