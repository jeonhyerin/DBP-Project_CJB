...
<?php
	...
	$num = $_GET['num'];
	$sql = 'SELECT * FROM franchise WHERE num='.$num;
	...
	<tr>
		...
		<table width=100%>
			<tr>
				...
				<td align=right>
				<?php
					$sql = "SELECT num FROM franchise WHERE num > $num LIMIT 1";
					$prev_num = mysqli_fetch_assoc($connect, $sql);
					
					if($prev_num['num'])
					{
						echo "<a href=\"franchise_read.php?num=".$prev_num['num']."\"><font color='white'>[이전]</font></a>";
					}
					...
					$sql1 = "SELECT num FROM franchise WHERE num < $num ORDER BY num DESC LIMIT 1";
					$next_num = mysqli_fetch_assoc($connect, $sql1);

					
					if($next_num['num'])
					{
						echo "<a href=\"franchise_read.php?num=".$next_num['num']."\"><font color='white'>[다음]</font></a>";;
					}
					else
					{
						echo "[다음]";
...
<?php
	$sql = 'UPDATE franchise SET hit=hit+1 WHERE num='.$num;
	$result = mysqli_query($connect, $sql);
